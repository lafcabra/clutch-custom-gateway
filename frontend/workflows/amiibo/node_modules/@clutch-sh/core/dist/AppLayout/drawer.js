...
import { Avatar as MuiAvatar, Drawer as MuiDrawer, List, ListItem, Typography, } from "@material-ui/core";
...
const Group = ({ heading, open = false, selected = false, updateOpenGroup, closeGroup, children, }) => {
    ...
    return (React.createElement(GroupList, { "data-qa": "workflowGroup" },
        React.createElement(GroupListItem, { button: true, selected: open || selected, ref: anchorRef, "aria-controls": open ? "workflow-options" : undefined, "aria-haspopup": "true", onClick: () => {
                updateOpenGroup(heading);
            } },
            React.createElement(Avatar, null, heading.charAt(0)),
            React.createElement(GroupHeading, { align: "center" }, heading),
            React.createElement(Popper, { open: open, onClickAway: closeGroup, anchorRef: anchorRef, id: "workflow-options" }, children))));
};
const Link = ({ to, text }) => {
...
const Drawer = () => {
    ...
    return (React.createElement(DrawerPanel, { "data-qa": "drawer", variant: "permanent" }, sortedGroupings(workflows).map(grouping => {
        const value = routesByGrouping(workflows)[grouping];
        const sortedWorkflows = _.sortBy(value.workflows, w => w.displayName);
        return (React.createElement(Group, { key: grouping, heading: grouping, open: openGroup === grouping, selected: openGroup === grouping || (activeWorkflow === null || activeWorkflow === void 0 ? void 0 : activeWorkflow.group) === grouping, updateOpenGroup: updateOpenGroup, closeGroup: () => setOpenGroup("") }, sortedWorkflows.map(workflow => (React.createElement(Link, { key: workflow.path.replace("/", ""), to: workflow.path, text: workflow.displayName })))));
    })));
};
