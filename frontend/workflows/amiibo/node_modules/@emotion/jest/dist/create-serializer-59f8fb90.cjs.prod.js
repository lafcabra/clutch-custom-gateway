...
function unwrapFromPotentialFragment(node) {
  return node.type() === Symbol.for("react.fragment") ? (!!node.dive && node.children().first().dive(), 
  node.children().last()) : node;
}

function getClassNamesFromEnzyme(selectors, nodeWithPotentialFragment) {
  var node = unwrapFromPotentialFragment(nodeWithPotentialFragment);
...
function isEmotionCssPropElementType(val) {
  return val.$$typeof === Symbol.for("react.element") && "EmotionCssPropInternal" === val.type.displayName;
}

function isStyledElementType(val) {
  if (val.$$typeof !== Symbol.for("react.element")) return !1;
  var type = val.type;
...
var tickledCssProps = new WeakMap, getTickledClassName = function(cssProp) {
...
var createConvertEmotionElements = function(keys) {
...
function createSerializer(_temp) {
  var _ref2 = void 0 === _temp ? {} : _temp, classNameReplacer = _ref2.classNameReplacer, _ref2$DOMElements = _ref2.DOMElements, DOMElements = void 0 === _ref2$DOMElements || _ref2$DOMElements, _ref2$includeStyles = _ref2.includeStyles, includeStyles = void 0 === _ref2$includeStyles || _ref2$includeStyles, cache = new WeakSet;
  return {
    test: function(val) {
    ...
    serialize: function(val, config, indentation, depth, refs, printer) {
      var elements = getStyleElements(), keys = getKeys(elements), converted = deepTransform(val, createConvertEmotionElements(keys)), nodes = getNodes(converted), classNames = getClassNamesFromNodes(nodes), styles = includeStyles ? getPrettyStylesFromClassNames(classNames, elements, config.indent) : "";
...
exports.isEmotionCssPropElementType = isEmotionCssPropElementType, exports.isStyledElementType = isStyledElementType, 
exports.tickle = tickle, exports.unwrapFromPotentialFragment = unwrapFromPotentialFragment;
